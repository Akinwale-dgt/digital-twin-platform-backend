...
async function generatePDF(reportData, reportId) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Extract Analysis section specifically for NetScore value
        ...
        narrativeText = narrativeText
          ...
          .filter((line) => {
            ...
            return (
              ...
              trimmedLine.startsWith('-') // Only include bullet points
            )
          })
          .map(line => line.substring(1).trim()) // Remove the bullet point character
          .join(' ')
      ...
      if (subjectiveMatch && subjectiveMatch.length > 0) {
        ...
        // Add bulleted list
        const subjectiveText = subjectiveMatch[1].trim();
        if (subjectiveText && subjectiveText.length > 0) {
          addBulletedList(doc, subjectiveText);
        } else {
          doc.text("No subjective input data available");
        }
      }
      ...
      if (objectiveMatch && objectiveMatch.length > 0) {
        ...
        if (objectiveText && objectiveText.length > 0) {
          addBulletedList(doc, objectiveText);
        } else {
          doc.text("No objective metrics data available");
        }
      }
      ...
      if (highlightsMatch && highlightsMatch.length > 0) {
        ...
        if (highlightsText.includes('- ')) {
          addBulletedList(doc, highlightsText);
        } else {
          doc.text(highlightsText, 50, doc.y, {
            align: 'left',
            width: doc.page.width - 100, // Leave margin on both sides
          });
        }
